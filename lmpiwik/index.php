...
*/
	...
	$oPluginAdmin  = new PluginAdmin('LMPiwik');
	...
	$piwikurl = trim($oPluginAdmin->plugin->getOption("globalpiwikurl"));
	$websiteid = intVal($oPluginAdmin->plugin->getOption("globalwebsiteid"));
	$tokenauth = trim($oPluginAdmin->plugin->getOption("globaltokenauth"));

	if($piwikurl && $websiteid && $tokenauth)
	{
		...
		{
			$fullurl = 'http://'.serverVar('HTTP_HOST').$piwikurl;
		}
		
		$websitename = '';

		$json = file_get_contents($fullurl.'index.php?module=API&method=SitesManager.getSiteFromId&idSite='.$websiteid.'&format=JSON'.$tokenauth);
		
		if($json != false)
		{
			$aSite = json_decode($json, true);
			
			if(is_array($aSite))
			{
				$websitename = $aSite[0]['name'];
			}
		}
		
		echo '<div class="dialogbox">';
		echo '<h4 class="light">Visits overview for the last 90 days for website '.$websitename.' (id: '.$websiteid.')</h4>';
		echo '</div>';
		echo '<div id="widgetIframe"><iframe width="100%" height="550" src="'.$piwikurl.'index.php?module=Widgetize&action=iframe&widget=1&moduleToWidgetize=VisitsSummary&actionToWidgetize=index&idSite='.$websiteid.'&period=range&date=last90&disableLink=1&widget=1'.$tokenauth.'" scrolling="no" frameborder="0" marginheight="0" marginwidth="0"></iframe></div>';
	}
		
	if($piwikurl)
	{
		echo '<div class="dialogbox">';
		echo '<h4 class="light">Piwik Dashbard</h4>';
		echo '<div>';
		echo '<p>The Piwik Dashboard is available <a href="'.$piwikurl.'">here</a>.</p>';
		echo '</div></div>';
	}
